...
- [x] `/api/*` を Express で実装し in-memory で状態遷移
- [x] 100〜400ms のランダム遅延を全 API に適用（Vitest 時は遅延なし）
- [x] 招待コード発行、入力、pending_approval、承認まで一連動作
- [x] 認可エラーの形を本番想定で統一 (他人データ取得は 403)
- [x] 通知文ダミー生成をテンプレート + 分岐 + 乱数で実装
- [x] チャット応答ダミー生成をキーワード分岐で実装 (100〜800ms 遅延)
- [x] チャット保持設定 (`none` / `30days` / `90days`) と期限削除を実装
- [x] Notifications API を使ったブラウザ通知検証を実装
...
- [x] Firebase Authentication（メール）と API の ID トークン検証
- [x] Firestore `users/{uid}` へのプロフィール upsert（ペア本体は未移行・in-memory のまま）
