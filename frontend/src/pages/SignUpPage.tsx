...
export function SignUpPage(): ReactElement {
  const { firebaseEnabled, signUpWithEmailPassword } = useAuth();
  ...
  if (!firebaseEnabled) {
    return (
      <main className="mx-auto max-w-md px-6 py-16">
        <p className="text-sm text-ink/70">Firebase の環境変数がまだありません。</p>
        <p className="mt-6">
  ...
  return (
    <main className="mx-auto max-w-md px-6 py-16">
      ...
      <h1 className="mt-3 font-serif text-2xl tracking-tight text-ink">新規登録</h1>
      <p className="mt-6 max-w-prose text-sm text-ink/80">メールアドレスとパスワードでアカウントを作ります。</p>
      ...
      >
        <div>
          <label className="block text-sm text-ink/80" htmlFor="signup-email">
            メール
          </label>
          <input
            id="signup-email"
        ...
        <div>
          <label className="block text-sm text-ink/80" htmlFor="signup-password">
            パスワード（6 文字以上）
          </label>
          <input
            id="signup-password"
